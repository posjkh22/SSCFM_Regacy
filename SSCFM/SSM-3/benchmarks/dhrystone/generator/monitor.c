...
#include "tiny_stack/aces_stack.h"
...
#define SIGNATURE_QUEUE_SIZE 23300
//#define SIGNATURE_QUEUE_SIZE 100000
...
//#define SIGNATURE_QUEUE_SIZE 1200000 // 1,000,000
...
unsigned int signature_queue_set[THREAD_NUMBER][SIGNATURE_QUEUE_SIZE];
unsigned int monitor_process_num = 0;

ReturnPointStack app_current_returnPointStack;
ReturnPointStack app_prior_returnPointStack;
...
monitor_dat monitor_data[THREAD_NUMBER];
...
{	
	...
	static unsigned thread_number = 0;
...
void initialize_monitor()
{
	...
	p_app_current_returnPointStack = &app_current_returnPointStack;
	//InitStack(p_app_current_returnPointStack, "app_current");
	InitStack(p_app_current_returnPointStack);
	
	//p_app_prior_returnPointStack = (Stack* )malloc(sizeof(Stack));
	p_app_prior_returnPointStack = &app_prior_returnPointStack;
	//InitStack(p_app_prior_returnPointStack, "app_prior");
...
void monitor_thread_generator()
{
	...
	monitor_dat* mon_dat = &(monitor_data[thread_number++]);
	...
	copy_stack(&(mon_dat->p_mon_returnPointStack), p_app_prior_returnPointStack);
...
void enqueue_signature(int i)
{
	...
	gettimeofday(&app_copy2_begTick, NULL);
		...
		copy_stack(p_app_prior_returnPointStack, p_app_current_returnPointStack);
